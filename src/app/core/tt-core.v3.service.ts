...
import { AmmoType, CardTypes, DBEnchant, DBEnchantTypes, DBFood, DBItem, DBItemCombo, DBJob, DBMob, DBMobClass, DBPet, DBSkill, DBSkillEnum, DBWeaponTypeEntry, DBWeaponTypeKey, ElementDBV3, EnchantDBV3, FoodCategory, FoodStatsNames, JSONFood, SkillMisc } from "./tt-models.v3";
...
export class TTCoreServiceV3 {
    ...
    readonly #dbFiles: { file: string, label: string }[] = [
        { file: 'assets/db/item.db.V3.json', label: 'Item database' },                  // 0
        { file: 'assets/db/job.db.V3.json', label: 'Job database' },                    // 1
        { file: 'assets/db/item-combo.db.V3.json', label: 'Item-Combo database' },      // 2
        { file: 'assets/db/mob.db.json', label: 'Monster datebase' },                   // 3
        { file: 'assets/db/skill.db.V3.json', label: 'Skill datebase' },                // 4
        { file: 'assets/db/element.db.json', label: 'Element datebase' },               // 5
        { file: 'assets/db/weapon-type.db.V3.json', label: 'Weapon-Type datebase' },    // 6
        { file: 'assets/db/ammo.db.json', label: 'Ammo datebase' },                     // 7
        { file: 'assets/db/food.db.V3.json', label: 'Food datebase' },                  // 8
        { file: 'assets/db/enchant.db.V3.json', label: 'Enchant datebase' },            // 9
        { file: 'assets/db/pet.db.V3.json', label: 'Pet datebase' },                    // 10
        { file: 'assets/db/item-custom.db.V3.json', label: 'Custom-Items datebase' },   // 11
    ];
    ...
    async initializeCore() {
        ...
        /* start loader */
        const loadingDone = this.#loader.start('Initialize core');

        /* create DB loaders */
        const dbPromises = this.#dbFiles.map(({ file, label }) => {
            return this.#loader.loadUntil(label, () => firstValueFrom(this.#http.get(file)))
        });

        /* wait for loading */
        const dbRes = await Promise.all(dbPromises);
