...
export class TTBattleSession {
  private _si!: SessionInfoV2;
  private _ammo: Ammo | undefined;
  private _target!: Mob;
  private _targetClass!: MobClass;
  private _rhWeapon!: Weapon;
  private _rhWeaponType!: WeaponType;
  private _lhWeapon!: Armor | Weapon;
  private _lhWeaponType!: WeaponTypeLeft;
  ...
  private _resultData: BattleSessionResult;
  private _result: BehaviorSubject<BattleSessionResult>;
  public result$: Observable<BattleSessionResult>;

  constructor(private core: TTCoreService, private session: TTSessionInfoV2Service) {
    this._str = 0;
    this._agi = 0;
    this._vit = 0;
    this._int = 0;
    ...
    this._result = new BehaviorSubject<BattleSessionResult>(this._resultData);
  ...
  async init(
    ...
    selectedAmmo: Ammo | undefined,
    appliedEndow: Element | undefined
  ): Promise<void> {
    this._resultData = {
      castTime: 0,
      hitRatio: 0,
    ...
    if (this._rhWeaponType)
      this._rhWeapon = this.core.weaponDbV2[this._rhWeaponType][this._si.equip.rightHand];
    ...
    if (this._lhWeaponType && 'Shield' === this._lhWeaponType)
      this._lhWeapon = this.core.shieldDbV2[this._si.equip.leftHand];
    else
      this._lhWeapon = this.core.weaponDbV2[this._lhWeaponType][this._si.equip.leftHand];

    this._ammo = selectedAmmo;
    this._appliedEndow = appliedEndow;
    this._activeSkill = activeSkill;
    this._activeSkillLv = activeSkillLv;
    this._baseLv = this._si.baseLevel;

    this._str = this.session.str;
    this._agi = this.session.agi;
    this._vit = this.session.vit;
    this._int = this.session.int;
    this._dex = this.session.dex;
    this._luk = this.session.luk;
  }
  ...
  public simulate(): number[] {
    ...
    // update result and publish
    this._resultData.minDamage = damage[0];
    this._resultData.maxDamage = damage[1];
    this._result.next(this._resultData);
  ...
  private calcBaseAtk(baseAtk: number, isCriticalAttack: boolean, isDualWielding: boolean, isDexBased: boolean) {
    let sizeModifier = isDualWielding
      ? this.core.waeponTypeDbV2[this._lhWeaponType as WeaponType].sizeModifier[this._target.size]
      : this.core.waeponTypeDbV2[this._rhWeaponType].sizeModifier[this._target.size];
    ...
    if (
      ...
      ('Spear' === this._rhWeaponType || 'Two-handed Spear' === this._rhWeaponType) &&
      this._target['size'] === 'medium' // is 1 === medium??
    ) {
      sizeModifier = this.core.waeponTypeDbV2[this._rhWeaponType].sizeModifier.large;
      //this.ttCoreService.weaponTypeDb[this._rhWeaponType]['large']; // TODO: what does this mean?? is the above line correct
    }
    ...
    let weaponLv =
      isDualWielding && this._lhWeapon
        ? (this._lhWeapon as Weapon).weaponLv
    ...
    let minMatk = this.session.minMatk;
    minDamage =
  ...
  private applyMasteryBonus(damage: number[]): number[] {
    ...
    if (this._activeSkill.enableMasteries) {  //TODO: whats that?! cant find it anywhere
      // Masteries related to weapons
      switch (this._rhWeaponType) {
        case 'Dagger': // Dagger
      ...
      if ('brute' === this._target.race || 'insect' === this._target.race) {
  ...
  private calcSkillBaseDamage(
    ...
    switch (this._activeSkill.id) {
      ...
      case 284: // Sacrifice#284
        skillBaseDamage = Math.floor((this.session.maxHp * 9) / 100); // TODO: is this correct? it was: this._si["maxHp"]
        break;
      ...
      case 438: // Final Strike [MaxHP - 1]#438
        skillBaseDamage = Math.floor(
          40 * this._str +
          ((this.session.maxHp - 1) * 8 * this._activeSkill.id) / 100
        );  // TODO: is this correct? it was: this._si["maxHp"]
        break;
      case 259: // Spiral Pierce#259
        let spearWeight = this._rhWeapon.weight;
        skillBaseDamage = Math.floor(spearWeight * 0.8) * (1 + 0.5 * this._activeSkillLv); // 80% of weapon's weight x ratio which only applies to weight
        ...
        // Apply size modifier
        skillBaseDamage = Math.floor(
          skillBaseDamage * (1.25 - MOB_SIZE_MODIFIER[this._target.size] * 0.25)
        );
        break;
    ...
    if ((394 == this._activeSkill.id || 395 == this._activeSkill.id) && this._ammo) // TODO: added if _ammo is defined
  ...
  private applyPhysicalDamageModifiers(
    ...
    if (this._activeSkill.allows_modifiers) {
      // bAddRace - physical damage modifier against race r
      let raceModifier = 100 + this._si.activeBonus.addRace[this._target.race];

      // bAddEle - Physical damage modifier against element e
      let elementModifier = 100 + this._si.activeBonus.addElement[this._target.element];

      // bAddSize - Physical damage modifier against size s
      let sizeModifier = 100 + this._si.activeBonus.addSize[this._target.size];
      ...
      let classModifier =
        100 +
        this._si.activeBonus.addClass[this._targetClass] +
  ...
  private applyPhysicalSkillDamageModifiers(damage: number[]): number[] {
    ...
    if (this._activeSkill.id == 86 && (50 <= n_B[3] && n_B[3] < 60))
      skillModifier += 30 * this._activeSkill.idLV;
  ...
  private applyMagicalSkillDamageRatio(damage: number[], skillId: number): number[] {
    // FIXME
  ...
  private applyElementDamageRatio(damage: number[]): number[] {
    ...
    let activeElement: SkillElement = 'neutral';
    ...
    if (!this._activeSkill.ignoreElement) {
      ...
      if ('weapon' === activeElement) {
        ...
        else activeElement = this._si.activeBonus.weaponElement;
      }
  ...
  private applyOffensiveStatusChange(damage: number[]): number[] {
    if (!this._activeSkill.ignoreOffensiveStatus) {
  ...
  private applyMiscDamageBonus(damage: number[]): number[] {
    ...
    if (423 == this._activeSkill.id)
      misc_damage_bonus = [this.session.minMatk, this.session.maxMatk];
  ...
  private isAttackPiercing(): boolean {
    if (
      ...
      return (
        ...
        this._si.activeBonus.defRatioAtkClass > 0 // TODO: Check for defRatioAtkClass > 0 ok correct?
  ...
  private applyPostDefenseDamageBonus(
    ...
    if (isDualWielding)
      weaponRefineBonus = this.calcWeaponRefineBonus(
        this._si.refine.leftHand,
        (<Weapon>this._lhWeapon).weaponLv
      );
  ...
  private calcMagicalAttackDamage(damage: number[]): number[] {
    ...
    damage[0] = this.session.minMatk;
    damage[1] = this.session.maxMatk;
    ...
    if (this._activeSkill.allows_modifiers) {
      // MagicAddEle
      let elementModifier = 100;

      if (this._activeSkill.element !== 'weapon') { // TODO: needed to check otherwise type problem
        // Damage modifier on magic element
        elementModifier = 100 + this._si.activeBonus.magicElementRate[this._activeSkill.element];

      }
      // Damage modifier for monster element
      elementModifier += this._si.activeBonus.magicAddElement[this._target.element];
      ...
      let classModifier =
        100 +
        this._si.activeBonus.magicAddClass[this._targetClass] +
        this._si.activeBonus.magicAddClass.all;

      // bMagicAddSize - unused modifier
      // bMagicAddRace2
      let magicAddRace2Modifier = 100;
      if (this._target['race2'])
        magicAddRace2Modifier += this._si['activeBonus']['magicAddRace2'][this._target['race2']];
  ...
  private calcAttackDamage(
    ...
    if (22 == this._activeSkill.id)
      ...
      // Pressure#283
      return damage.map(() => 500 + 300 * this._activeSkillLv);
    else if (397 == this._activeSkill.id) {
      // Throw Zeny#397
      let zeny_min_damage = 500 * this._activeSkillLv;
    ...
    console.log(damage);
     ...
     let hits: number = 1;
     let misc_flag: number | null = null; // FIXME: Is misc flag required for hits in skill db ?
  ...
  private calcPhysicalAttackDamage(
    ...
    let baseAtk = this.session.baseAtk;
    let isDexBased: boolean = 'ammoType' in this.core.waeponTypeDbV2[this._rhWeaponType];
